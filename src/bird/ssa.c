...
static void postorder_visit(LIST(long) postorder, ir_blk_t *blk)
{
	...
	blk->visited = true;
	if(!has_long(postorder, blk->num)) {
		list_append(postorder, blk->num);
	}

	if(blk->tail->true_blk) {
		if(!has_long(postorder, blk->tail->true_blk->num)) {
			list_append(postorder, blk->tail->true_blk->num);
		}
		postorder_visit(postorder, blk->tail->true_blk);
	}
	if(blk->tail->false_blk) {
		if(!has_long(postorder, blk->tail->false_blk->num)) {
			list_append(postorder, blk->tail->false_blk->num);
		}
...
{
	return reg->ssareg ? reg->ssareg : reg;
}
...
static reg_t *write_reg(ir_blk_t *blk, reg_t *reg)
{
	reg_t *var = find_var(reg);
	reg_t *new = ssa_tmp(var);

	if(!has_blkreg(var->blkregs, blk)) {
		list_append(var->blkregs, blkreg_make(blk, new));
	} else {
		var->blkregs[find_blkreg(var->blkregs, blk)]->reg = new;
	}
...
static reg_t *read_var_rec(ir_blk_t *blk, reg_t *reg)
{
	...
	if(list_len(blk->pred) == 1) {
		...
		ir_inst_t *phi = insert_phi(blk);
		phi->r0 = write_reg(blk, reg);
		val = add_phi_ops(blk, reg, phi);
	}
	return val;
}

static reg_t *read_reg_gvn_normal(ir_blk_t *blk, reg_t *reg)
{
	reg_t *var = find_var(reg);
	if(has_blkreg(var->blkregs, blk)) {
		if(reg == var) {
...
void ir_ssa_enter(ir_func_t *fun)
{
	...
	for(size_t ip = 0; ip < list_len(postorder); ip++) {
		...
		for(ir_inst_t *inst = blk->insts; inst; inst = inst->next) {
			if(inst->r0) {
				inst->r0 = write_reg(blk, inst->r0);
			}
			if(inst->r1) {
				inst->r1 = read_reg_lvn(blk, inst->r1);
			}
			if(inst->r2) {
				inst->r2 = read_reg_lvn(blk, inst->r2);
			}
			if(inst->type == IR_INST_CALL) {
				for(size_t i = 0; i < list_len(inst->call_args); i++) {
					inst->call_args[i]->r =
						read_reg_lvn(blk, inst->call_args[i]->r);
				}
			}
		}
	}
