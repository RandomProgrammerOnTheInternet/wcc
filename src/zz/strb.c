...
static void strb_fit(strb_t *strb, size_t to, size_t add)
{
	if(strb->cap) {
...
strb_t strb_make(char *initial_str, size_t len)
{
	strb_t strb = { 0 };
	strb.len = len;
	strb_fit(&strb, strb.len, STRB_GROWTH_RATE);
	if(initial_str) {
...
{
	strb_fit(strb, strb->len + len, STRB_GROWTH_RATE);
...
{
	strb_fit(strb, strb->len + 1, STRB_GROWTH_RATE);
